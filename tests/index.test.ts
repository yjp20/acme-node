...
import { APIUserAbortError } from 'test-acme';
import { Headers } from 'test-acme/core';
...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new TestAcme({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    test('explicit option', () => {
      const client = new TestAcme({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new TestAcme({ apiKey: 'My API Key', environment: 'production' }),
      ...
      const client = new TestAcme({ apiKey: 'My API Key', baseURL: null, environment: 'production' });
      expect(client.baseURL).toEqual('https://api.acme.com/v1');
    });
  });

  test('maxRetries option is correctly set', () => {
    const client = new TestAcme({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new TestAcme({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  test('with environment variable arguments', () => {
    // set options via env var
    process.env['TEST_ACME_API_KEY'] = 'My API Key';
    const client = new TestAcme();
  ...
  test('with overriden environment variable arguments', () => {
    // set options via env var
    process.env['TEST_ACME_API_KEY'] = 'another My API Key';
    const client = new TestAcme({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
