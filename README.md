# Test Acme Node API Library

[![NPM version](https://img.shields.io/npm/v/test-acme.svg)](https://npmjs.org/package/test-acme)
...
const testAcme = new TestAcme({
  apiKey: process.env['TEST_ACME_API_KEY'], // This is the default and can be omitted
  environment: 'environment_1', // defaults to 'production'
});

async function main() {
  const statusRetrieveResponse = await testAcme.status.retrieve();
...
async function main() {
  const statusRetrieveResponse: TestAcme.StatusRetrieveResponse = await testAcme.status.retrieve();
}
...
async function main() {
  const status = await testAcme.status.retrieve().catch((err) => {
    if (err instanceof TestAcme.APIError) {
...
await testAcme.status.retrieve({
  maxRetries: 5,
});
```
...
const response = await testAcme.status.retrieve().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: statusRetrieveResponse, response: raw } = await testAcme.status.retrieve().withResponse();
...
- Deno v1.28.0 or higher, using `import TestAcme from "npm:test-acme"`.
- Bun 1.0 or later.
- Cloudflare Workers.
- Vercel Edge Runtime.
